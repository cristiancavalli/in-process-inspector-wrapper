...
const ScriptSourceSetsQueue = require('../../../lib/InspectorClient/debugger/ScriptSourceSetsQueue');
...
describe('ScriptSourceSetsQueue', function () {
  const id = 0;
  const scriptId = 'test';
  const scriptSource = 'here';
  var inst;
  beforeEach(() => inst = new ScriptSourceSetsQueue(id));
  after(() => ScriptSourceSetsQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of SetScriptSourceCommand',
      () => assert(inst.newCommand(id, scriptId, scriptSource) instanceof SetScriptSourceCommand));
  });
  describe('Adding commands', function () {
    describe('Valid behaviour', function () {
      it('Should allow the adding of valid commands', function () {
        const id = 0;
        const payload = inst.newCommand(id, scriptId, scriptSource);
