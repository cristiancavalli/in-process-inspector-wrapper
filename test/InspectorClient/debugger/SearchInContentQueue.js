...
const SearchInContentQueue = require('../../../lib/InspectorClient/debugger/SearchInContentQueue');
...
describe('SearchInContentQueue', function () {
  const id = 0;
  const scriptId = 'test';
  const query = '/test/';
  var inst;
  beforeEach(() => inst = new SearchInContentQueue(id));
  after(() => SearchInContentQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of SearchInContentCommand',
      () => assert(inst.newCommand(id, scriptId, query) instanceof SearchInContentCommand));
  });
  describe('Adding commands', function () {
    describe('Valid behaviour', function () {
      it('Should allow the adding of valid commands', function () {
        const id = 0;
        const payload = inst.newCommand(id, scriptId, query);
