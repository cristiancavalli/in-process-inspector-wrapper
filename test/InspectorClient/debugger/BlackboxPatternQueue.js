...
const BlackboxPatternQueue = require('../../../lib/InspectorClient/debugger/BlackboxPatternQueue');
const SetBlackboxPatternsCommand = require('../../../lib/InspectorClient/debugger/commands/SetBlackboxPatternsCommand');
const InvalidTypeGivenForCommand = require('../../../lib/InspectorClient/errors/InvalidTypeGivenForCommand');

describe('BlackboxPatternQueue', function () {
  const id = 0;
  var inst;
  beforeEach(() => inst = new BlackboxPatternQueue(id));
  after(() => BlackboxPatternQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of SetBlackboxPatternsCommand',
      () => assert(inst.newCommand(0, ['test']) instanceof SetBlackboxPatternsCommand));
  });
  describe('Adding commands', function () {
    describe('Valid behaviour', function () {
      it('Should allow the adding of valid commands', function () {
        const id = 0;
        const payload = inst.newCommand(id, ['test']);
