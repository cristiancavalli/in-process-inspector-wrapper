...
const FrameRestartCommand = require('../../../../lib/InspectorClient/debugger/commands/FrameRestartCommand');

describe('FrameRestartCommand', function () {
  const id = 0;
  const cfId = 'test';
  const template = FrameRestartCommand;
  describe('Initialization', function () {
    ...
    describe('Request parameter population', function () {
      it('Should populate callFrameId param',
        () => assert.deepEqual(
          new template(id, cfId).requestPayload().params,
          {callFrameId: cfId}
        )
      );
    });
