...
describe('SetBreakpointCommand', function () {
  const id = 0;
  const location = {scriptId: 'x', lineNumber: 0, columnNumber: 0};
  const condition = '1+1';
  const template = SetBreakpointCommand;
  describe('Initialization', function () {
    ...
    describe('Request parameter population', function () {
      it('Should populate location param',
        () => assert.deepEqual(
      ...
      it('Should populate condition param',
        () => assert.deepEqual(
          new template(id, location, condition).requestPayload().params,
