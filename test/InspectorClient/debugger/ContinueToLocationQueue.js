...
const ContinueToLocationQueue = require('../../../lib/InspectorClient/debugger/ContinueToLocationQueue');
...
describe('ContinueToLocationQueue', function () {
  const id = 0;
  const location = {scriptId: 'x', lineNumber: 1};
  var inst;
  beforeEach(() => inst = new ContinueToLocationQueue(id));
  after(() => ContinueToLocationQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of ContinueToLocationCommand',
      () => assert(inst.newCommand(0, location) instanceof ContinueToLocationCommand));
  });
