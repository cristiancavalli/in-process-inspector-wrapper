...
const StepOverQueue = require('../../../lib/InspectorClient/debugger/StepOverQueue');
...
describe('StepOverQueue', function () {
  const id = 0;
  var inst;
  beforeEach(() => inst = new StepOverQueue(id));
  after(() => StepOverQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of StepOverCommand',
      () => assert(inst.newCommand(0, 0) instanceof StepOverCommand));
  });
