...
const VariableSetQueue = require('../../../lib/InspectorClient/debugger/VariableSetQueue');
...
describe('VariableSetQueue', function () {
  const id = 0;
  var inst;
  beforeEach(() => inst = new VariableSetQueue(id));
  after(() => VariableSetQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of VariableSetCommand',
      () => assert(inst.newCommand(id, 1, 'test', {}, '1') instanceof VariableSetCommand));
  });
  describe('Adding commands', function () {
    describe('Valid behaviour', function () {
      it('Should allow the adding of valid commands', function () {
        const id = 0;
        const payload = inst.newCommand(id, 1, 'test', {}, '1');
