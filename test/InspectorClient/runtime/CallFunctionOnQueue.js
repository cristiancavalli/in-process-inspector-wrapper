...
const CallFunctionOnQueue = require('../../../lib/InspectorClient/runtime/CallFunctionOnQueue');
...
describe('CallFunctionOnQueue', function () {
  const id = 0;
  const objectId = 'test';
  const functionDeclaration = 'x';
  const bldr = new CallFunctionOnQueue().newFunctionCallBuilder(objectId,
    functionDeclaration);
  const cmd = new CallFunctionOnQueue().newCommand(id, bldr);
  var inst;
  beforeEach(() => inst = new CallFunctionOnQueue());
  after(() => CallFunctionOnQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of CallFunctionOnCommand',
      () => assert(inst.newCommand(id, bldr) instanceof CallFunctionOnCommand));
  });
