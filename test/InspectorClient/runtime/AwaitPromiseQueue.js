...
const AwaitPromiseQueue = require('../../../lib/InspectorClient/runtime/AwaitPromiseQueue');
const AwaitPromiseCommand = require('../../../lib/InspectorClient/runtime/commands/AwaitPromiseCommand');
const InvalidTypeGivenForCommand = require('../../../lib/InspectorClient/errors/InvalidTypeGivenForCommand');

describe('AwaitPromiseQueue', function () {
  const id = 0;
  const promiseObjectId = 'test';
  const returnByValue = true;
  const generatePreview = true;
  const cmd = new AwaitPromiseCommand(id, promiseObjectId, returnByValue,
    generatePreview);
  var inst;
  beforeEach(() => inst = new AwaitPromiseQueue());
  after(() => AwaitPromiseQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of AwaitPromiseCommand',
      () => assert(inst.newCommand(id, promiseObjectId, returnByValue,
        generatePreview) instanceof AwaitPromiseCommand));
  });
