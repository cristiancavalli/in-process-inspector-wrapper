...
const ReleaseObjectQueue = require('../../../lib/InspectorClient/runtime/ReleaseObjectQueue');
const ReleaseObjectCommand = require('../../../lib/InspectorClient/runtime/commands/ReleaseObjectCommand');
const InvalidTypeGivenForCommand = require('../../../lib/InspectorClient/errors/InvalidTypeGivenForCommand');

describe('ReleaseObjectQueue', function () {
  const id = 0;
  const objectId = 'test';
  const cmd = new ReleaseObjectQueue().newCommand(id, objectId);
  var inst;
  beforeEach(() => inst = new ReleaseObjectQueue());
  after(() => ReleaseObjectQueue.data().reset());
  describe('Generating new commands', function () {
    it('Should generate instances of ReleaseObjectCommand',
      () => assert(inst.newCommand(id, objectId) instanceof ReleaseObjectCommand));
  });
