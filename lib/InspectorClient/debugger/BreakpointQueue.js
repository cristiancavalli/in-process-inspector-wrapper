...
class BreakpointQueue extends RPCCommandCollection {
  ...
  searchForMatchingBreakpoint (msg) {
    if (!msg.params || !msg.params.callFrames) {
      return;
    }
    const data = RPCCommandCollection.data();
    const msgLocation = first(msg.params.callFrames).location;
    ...
    forEach(data.getTable(), (cmd, msgId) => {
      ...
      const pooledLocation = cmd.responsePayload().actualLocation;
      const locationsMatch = every([pooledLocation], msgLocation);
