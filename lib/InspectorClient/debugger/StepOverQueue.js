...
class StepOverQueue extends RPCCommandCollection {
  constructor () {
    super();
  }
  newCommand (messageId) {
    return new StepOverCommand(messageId);
  }
  addCommand (messageId, entry) {
    if (entry instanceof StepOverCommand) {
      super.addCommand(messageId, entry);
      return this;
    }
    throw new InvalidTypeGivenForCommand(StepOverCommand.name);
  }
}

module.exports = StepOverQueue;
